 ...
 * the LIVE parser — `SqllensDocumentParser` — testing the engine that ships
 * in production.
 *
 ...
 * as a runtime bug (the lineage 0-deps class). sqllens is synchronous and fast,
 * allowing thorough assertions on real-world models.
 *
...
describe('sample project DocumentModel (sqllens — live engine)', () => {
	...
	for (const filePath of SQL_FILES) {
		...
		it(label, async () => {
			...
			RESULTS.push({
				label,
				warnings: (model.parseWarnings ?? []).length,
				extraction: {
