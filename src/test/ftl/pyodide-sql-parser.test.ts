...
SELECT mkey, sourcename FROM warehouse`;
	...
	it('sqlTokens carry comment spans for trailing -- comment containing function name (regression: count in comment)', async () => {
		// Regression: layout-function-spacing was firing on "count (conversion...)" inside a -- comment.
		// Root cause: sqlTokens.comments must be populated so the rule can mask the comment span.
		// This is the exact SQL from the reported bug.
		const sql = 'when is_display_pallet then qty  -- Display qty IS pallet count (conversion used later)\n';
		const result = await parser.parse(sql, 'duckdb');
		expect(result.sqlTokens, 'sqlTokens must be present').toBeDefined();
		const tokenWithCountComment = result.sqlTokens!.find(t =>
			t.comments?.some(c => c.text.includes('count')),
		);
		expect(tokenWithCountComment, 'No token carries a comment containing "count" — layout rule comment masking will not work').toBeDefined();
		const commentStart = sql.indexOf('--');
		const comment = tokenWithCountComment!.comments!.find(c => c.text.includes('count'))!;
		expect(comment.start).toBe(commentStart);
	});

	it('layout-function-spacing rule does not fire for function name inside -- comment (end-to-end regression)', async () => {
		// End-to-end: real parser → real sqlTokens → layout rule → must produce zero violations.
		// This is the exact case from the bug report: "count (conversion...)" is inside a -- comment.
		const { functionSpacingRule } = await import('../../ninja/rules/layout-function-spacing.js');
		const { DEFAULT_CONFIG } = await import('../../ninja/config.js');
		const sql = 'when is_display_pallet then qty  -- Display qty IS pallet count (conversion used later)\n';
		const result = await parser.parse(sql, 'duckdb');
		...
		const violations = functionSpacingRule.check({
			text: sql,
			lines: sql.split('\n'),
			jinjaTokens: [],
			document: doc,
	...
	it('sqlglot attaches -- comment text to the preceding token, not as a separate COMMENT token', async () => {
		// sqlglot's tokenizer does NOT emit a standalone COMMENT token.
		// Instead it appends the comment text to the preceding token's `token.comments` list.
		// _tokenize() in sql_parser.py post-processes the raw text to compute
		// the exact [start, end) span of each comment so callers can do
		// position-based "is offset X inside a comment?" checks.
		//
		// "select x  -- count (foo)\n"
		//  offsets:  7    10        24
		//  x → VAR@7-7, comment span [10, 24) attached to VAR
		const sql = 'select x  -- count (foo)\n';
		const result = await parser.parse(sql, 'duckdb');
		const varTok = result.sqlTokens!.find(t => t.type === 'VAR' && t.start === 7);
		expect(varTok, 'VAR token for x not found').toBeDefined();
		expect(varTok!.comments, '_tokenize must serialize token.comments').toBeDefined();
