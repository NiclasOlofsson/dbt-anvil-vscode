...
describe(RULE, () => {
	it('flags table aliased to its own name', () => {
		const tableSym = sym('table', 'orders', 0, 0, { alias: { name: 'orders', line: 0, col: 7 } });
	...
	it('ignores table aliased to different name', () => {
		const tableSym = sym('table', 'orders', 0, 0, { alias: { name: 'o', line: 0, col: 7 } });
	...
	it('ignores subquery alias — (...) AS po has name=alias but kind is subquery, not table/cte', () => {
		...
		const subquerySym = sym('subquery', 'po', 5, 5, { alias: { name: 'po', line: 5, col: 8 } });
