/**
 * Map sqllens syntax diagnostics to the extension's `SqlglotWarning[]`.
 *
 * Only `syntax_error` is produced. The sqlglot-era `scope_warning` ("SQL parsed
 * but a CTE scope can't be analysed") has NO sqllens analog — `resolveScopes()`
...
export function mapDiagnostics(diagnostics: SyntaxDiagnostic[]): SqlglotWarning[] {
