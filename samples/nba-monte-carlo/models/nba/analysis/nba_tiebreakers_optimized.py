...
def calculate_point_differential(
    ...
    return home_diff + away_diff  # type: ignore
...
def _get_record_against_teams(
    ...
    for opponent in opponent_list:
        if team == opponent:  # Don't compare team against itself
            continue
        w, l = all_h2h_records.get((team, opponent), (0, 0))  # noqa: E741
        wins += w
        losses += l
    return wins, losses
...
def model(dbt, sess):
    ...
    team_map = dict(zip(teams["team"].to_list(), teams["team_long"].to_list()))  # noqa: F841
    ...
    standings = standings.join(
        teams.select(["team", "conf", "division"]), on="team", how="left"
    )
